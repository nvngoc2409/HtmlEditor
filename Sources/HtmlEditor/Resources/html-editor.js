/*! Summernote v0.8.1 | (c) 2013-2015 Alan Hong and other contributors | MIT license */ !(function (
    ...
    q = function (b, e) {
      ...
      return (
        ...
        (this.createInvokeHandler = function (b, c) {
          return function (d) {
            d.preventDefault();
            const domClasslist = d.currentTarget.classList;
            if (d.type === 'click' && checkClassList(domClasslist)) {
              const element = a(d.currentTarget);
              const active = element.hasClass('active');
              g.toggleBtnActive(element, !active);
            }
      ...
      (this.fromNode = function (a) {
        ...
        (this.current = function (b) {
          ...
          try {
            ...
            d = a.extend(d, {
              'font-bold': isBold ? 'bold' : 'normal',
              'font-italic': isItalic ? 'italic' : 'normal',
              'font-underline': isUnderline ? 'underline' : 'normal',
      ...
      (this.events = {
        ...
        (this.initialize = function () {
          this.needKeydownHook()
            ? ((this.$paste = a('<div />').attr('contenteditable', !0).css({
                position: 'absolute',
                left: -1e5,
                opacity: 0,
              })),
  ...
  function checkClassList(array) {
    if (!array.length) return;

    const classList = [...array];
    const arrayBtn = ['note-btn-bold', 'note-btn-italic', 'note-btn-underline'];

    return classList.some((item) => arrayBtn.includes(item));
  }

  function checkIsButtonFontWeight(name) {
    ...
    return listClassName.includes(name);
  }
});
